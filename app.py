...
genai.configure(api_key="AIzaSyCFQnGFIcNi6sTBuRJXBHSzwh90cmYpMVw")
...
def run(video_path, model_name):
    ...
    for frame in reader: # type: ignore
        ...
        if is_accident_detected and i % skip_frames == 0 and take_snap < total_snap:
            filename = f"temp/time-{timestamp:.2f}s.jpg"
    ...
    explaination = describe_image(glob.glob("temp/*.jpg"))
    
    return output_path, glob.glob("temp/*.jpg"), explaination



inp_vid = gr.Video(height=400)
dropdown = gr.Dropdown(choices=['YOLOv8n', 'YOLOv8m', 'YOLOv8l'], value='YOLOv8n')
out_vid = gr.PlayableVideo(height=400)

gallery = gr.Gallery(label="Accident Detected images", show_label=True, columns=[3], object_fit="contain", height="auto", allow_preview=True, render=isGallaryVisible)
text_box = gr.TextArea(render=isGallaryVisible)
